...
public class CustomerPersistenceMapper : IPersistenceMapper<Customer, CustomerPersistenceDto>
{
    #region IMapper implementation

    public Customer ToDomain(CustomerPersistenceDto customerDto)
    {
    ...
    public void CopyFromDomain(Customer customer, CustomerPersistenceDto customerDto)
    {
    ...
    public CustomerPersistenceDto FromDomain(Customer customer)
    {
        ArgumentNullException.ThrowIfNull(customer, nameof(customer));
    ...
    public List<Customer> ToDomain(IEnumerable<CustomerPersistenceDto> customerDtos) =>
        customerDtos?.Select(ToDomain).ToList() ?? [];

    public List<CustomerPersistenceDto> FromDomain(IEnumerable<Customer> customers) =>
        customers?.Select(FromDomain).ToList() ?? [];

    #endregion IMapper implementation

    private static void Copy(Customer customer, CustomerPersistenceDto customerDto)
    {
